...
prBin <- predictionStats_binary(cbind(TADPOLE_DX_TEST$DX,predict(LASSODXmlNLDE,TADPOLE_DX_NLDE_TEST)),"NL vs Dementia")
pander::pander(prBin$aucs)
pander::pander(prBin$accc)
pander::pander(prBin$berror)
pander::pander(prBin$sensitivity)
...
prBin <- predictionStats_binary(cbind(TADPOLE_DX_TESTD$DX,predict(LASSODXmlNLDED,TADPOLE_DX_NLDE_TESTD)),"NL vs Dementia")
pander::pander(prBin$aucs)
pander::pander(prBin$accc)
pander::pander(prBin$berror)
pander::pander(prBin$sensitivity)
...
```{r results = "asis", warning = FALSE, dpi=600, fig.height= 6.0, fig.width= 8.0}
bConvmlD <- LASSO_1SE(Surv(TimeToEvent,status)~1,TADPOLE_Conv_TRAIND)
